...
### Serialization Layer
Serialization available through the `JsonSerializer`, supports a select few types, see `JsonSerializer` for supported types. Deserialization available through the `JsonDeserializer`, supports a select few types, see `JsonDeserializer` for supported types. Intended to simplify converting to and from C# types and JSON.
